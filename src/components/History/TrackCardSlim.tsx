...
const TrackCardSlim = ({ data }: Props) => {
	...
	const time = moment(new Date(data.timestamp)).fromNow();

	return (
		<div className="flex-bc overflow-hidden pl-4 mb-6 w-full h-28 bg-white bg-opacity-10 rounded-lg backdrop-blur group">
			<div className="flex-cs mr-4">
				...
				<div className="flex-ss col w-full">
					...
					<p className="text-sm text-right text-theme-blue">{time}</p>
				</div>
			</div>
		</div>
	);
